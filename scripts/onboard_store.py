...
async def main() -> None:
    ...
    print()
    print("Next steps:")
    print(f"  1. Tune the daily sheet fields to match their actual paper sheet (YOU do this):")
    print(f"       docker compose exec app python scripts/manage_store.py")
    print(f"     → Edit each field: label, auto vs manual, order, add/remove")
    print(f"     → Get it right before touching the Google Sheet")
    print()
    print(f"  2. Set up their Google Sheet (once the daily sheet fields are finalized):")
    print(f"     → Duplicate Moraine's sheet as a starting point (File → Make a copy)")
    print(f"     → Adjust tabs/columns to match this store's field layout")
    print(f"     → Share it with the service account:")
    try:
        import json as _j
        _creds = _j.loads(Path("config/google_credentials.json").read_text())
        print(f"         {_creds.get('client_email', '(see config/google_credentials.json)')}")
    except Exception:
        print(f"         (see config/google_credentials.json for service account email)")
    print(f"     → Update sheet column mappings to match the finalized fields:")
    print(f"         docker compose exec app python scripts/manage_store.py")
    print(f"         → choose 'Sheet column mappings'")
    print()
    print(f"  3. Create the owner's dashboard login:")
    print(f"       docker compose exec app python scripts/create_user.py")
    print(f"  4. Restart the bot:")
    print(f"       docker compose restart app")
    print(f"  5. Send /daily from the store's Telegram chat to verify everything looks correct.")
    print()
